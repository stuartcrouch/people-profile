...
//print_r($node->ouprofile['pdata']['group_layout_design']);
global $user;
...
//May need to add below to OUBS page
    ...
    if ($node->ou_profile['pdata']['group_header']['field_oup_job_title']['value'] || $node->ou_profile['pdata']['group_header']['field_pdat_appt_job_title']['value']) {
	  ...
	  //     }
	if (isset($node->ou_profile['pdata']['group_header']['field_pdat_l1_estab_unit_desc']) &&
	    isset($node->ou_profile['pdata']['group_header']['field_pdat_l1_estab_unit_desc']['value'])){
	      $profile_summary .= "</li>" . $node->ou_profile['pdata']['group_header']['field_pdat_l1_estab_unit_desc']['value'].'</li>';
	}
	if (isset($node->ou_profile['pdata']['group_header']['field_pdat_l1_estab_unit_name']) &&
	    isset($node->ou_profile['pdata']['group_header']['field_pdat_l1_estab_unit_name']['value'])) {
        $profile_summary .= "<li>".$node->ou_profile['pdata']['group_header']['field_pdat_l1_estab_unit_name']['value']."</li>";
    }
    if (isset($node->ou_profile['pdata']['group_header']['field_pdat_l2_estab_unit_name']) && 
        isset($node->ou_profile['pdata']['group_header']['field_pdat_l2_estab_unit_name']['value'])) {
        $profile_summary .= "<li>".$$node->ou_profile['pdata']['group_header']['field_pdat_l2_estab_unit_name']['value']."</li>";
    }
	if (isset($node->ou_profile['pdata']['group_header']['field_pdat_l3_estab_unit_name']) && 
	    isset($node->ou_profile['pdata']['group_header']['field_pdat_l3_estab_unit_name']['value'])) {
        $profile_summary .= "<li>".$node->ou_profile['pdata']['group_header']['field_pdat_l3_estab_unit_name']['value']."</li>";
    }
    if (isset($node->ou_profile['pdata']['group_header']['field_pdat_l4_estab_unit_name']) && 
	    isset($node->ou_profile['pdata']['group_header']['field_pdat_l4_estab_unit_name']['value'])) {
        $profile_summary .= "<li>".$node->ou_profile['pdata']['group_header']['field_pdat_l4_estab_unit_name']['value']."</li>";
    }
    if (isset($node->ou_profile['pdata']['group_header']['field_pdat_ou_email_address']) && 
		isset($node->ou_profile['pdata']['group_header']['field_pdat_ou_email_address']['value'])) {
        $profile_summary .= '<li><a href="mailto:'.$node->ou_profile['pdata']['group_header']['field_pdat_ou_email_address']['value'].'@open.ac.uk">'.$node->ou_profile['pdata']['group_header']['field_pdat_ou_email_address']['value'] ."</a></li>\n";
    }
	...
	if ($node->ou_profile['pdata']['group_teaching_research_interest'])
	{
		...
		foreach ($other_links as $link_field) {
			if ($weblink = $node->ou_profile['pdata']['group_external_profiles_accounts'][$link_field]['value']['url']) {
				$weblinks .= '<li class="'.$link_field.'"><a href="'.$weblink.'">'.$node->ou_profile['pdata']['group_external_profiles_accounts'][$link_field]['label'].'</a></li>';
			}
		}
		// including Twitter
